...
export const Uploader = () => {
  ...
  return (
    <div className="absolute bottom-6 right-6 z-50 flex gap-4 items-end">
      ...
      <div className="group">
        <label 
          className={`cursor-pointer backdrop-blur-md p-4 rounded-full shadow-lg transition-all flex items-center justify-center gap-2 group-hover:scale-105 
            ${isPlaying 
        ...
        >
          <svg 
            ...
            strokeWidth={1.5} 
            stroke="currentColor" 
            className={`w-6 h-6 transition-colors ${isPlaying ? 'text-yellow-400 animate-[spin_4s_linear_infinite]' : 'text-white'}`}
          >
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 9l10.5-3m0 6.553v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 11-.99-3.467l2.31-.66a2.25 2.25 0 001.632-2.163zm0 0V2.25L9 5.25v10.303m0 0v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 01-.99-3.467l2.31-.66A2.25 2.25 0 009 15.553z" />
          </svg>
          <span className={`font-cinzel text-sm hidden group-hover:block transition-all whitespace-nowrap ${isPlaying ? 'text-yellow-100' : 'text-white'}`}>
            {isPlaying ? 'Playing...' : 'Add Music'}
          </span>
          <input 
            type="file" 
            accept="audio/*" 
